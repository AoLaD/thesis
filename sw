...
Dat (datagramů). Síťová vrstva je pak schopna přenášet data v jedné síti nebo mezi vícero sítěmi i technologicky rozdílných.
...
Internet Protocol je zodpovědný za směrování datagramů (paketů) ze zdrojového zařízení do cílového zařízení přes jednu nebo více IP sítí. Datagram se skládá z řídících dat a z uživatelských dat. Řídící data zajišťují informace k doručení datagramů (adresu zdroje a cíle, kontrolní součty, informace o pořadí atd.).

Datagramy putují sítí zcela nezávisle. IP protokol v doručování datagramů používá nespolehlivé spojení (službu). To znamená, že všechny zařízení na síti se snaží datagram doručit do cíle podle svých možností, ale prakticky za nic neručí.
...
Transportní vrstva obsahuje, jak jinak, větší množství protokolů. Zde uvedeme jen dva protokoly a to TCP a UDP.
...
Úkolem prezentační vrstvy je transformovat data do tvaru, které používají aplikace. Dopsat doma z knihy
...
Stejně jako na nižších vrstvách i na aplikační vrstvě se můžeme setkat s různými protokoly. Tyto protokoly mohou být vázány ke konkrétní aplikaci, ale existují i aplikační protokoly, které jsou v některých případech nezbytné pro správný chod sítě a s aplikací běžící na aplikační vrstvě nemají nic

společného. Případem takového protokolu je například DHCP protokol (Dynamic Host Configuration Protocol). Dalšími známějšími protokoly aplikační vrstvy jsou protokoly HTTP, IMAP, SSH, POP3 a další.
