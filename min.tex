...
Můžeme zde naleznout návod, jak připravit hlavičkové soubory k mikrokontroléru, postup implementace základní funkcionality nového mikrokontroléru v projektu RTEMS a popis vývoje nového ovladače do knihovny LwIP.
...
To by mohlo vést k implementaci plné výjimek a přepínačů, která by se postupně nabalovala při každém přizpůsobení novému mikrokontroléru.
...
V praktických aplikacích je často požadavek, aby se do mikrokontroléru dala nahrát aktualizace stávající aplikace.
...
Kromě této funkcionality bývá zavaděč schopen komunikovat po vybrané komunikační sběrnici, po které dokáže přijímat příkazy nebo aktualizace kódu.
	
	Jádro CORTEX-R podporuje dvě možné umístění tabulek výjimek.
...
Druhá možnost jádra CORTEX-R4 je používat tabulku výjimek z adresy 0xFFFF0000.
Používat druhou tabulku výjimek na adrese 0xFFFF0000 je bohužel v mikrokontroléru TMS570 nemožné, protože adresa 0xFFFF0000 je rezervovaná pro periferie a není možné z ní vykonávat kód.
Toto je velmi důležité, protože operační systém RTEMS dokáže zpracovat všechny výjimky z tabulky výjimek a je vhodné, aby to také prováděl.
Avšak protože je nutné kód zavaděče vykonat jako první, nachází se zavaděč pravděpodobně v prostorech okolo tabulky výjimek a tabulka výjimek odkazuje na zavaděč.

	Měnit obsah tabulky výjimek je značně komplikované a to z toho důvodu, že se tabulka nachází v paměti typu flash. Počet zápisů do paměti typu flash je omezen.
Jednoduchým řešením tohoto problému je rozšířit zavaděč tak, aby skoky z tabulky výjimek přeposílal do aplikace.
Nevýhodou tohoto řešení je i několika skokové zpoždění reakční doby na přerušení nebo jinou výjimku.
Na mikrokontroléru TMS570 jde tento problém vyřešit pomocí periferie POM, která může tabulku výjimek překrýt virtuální stránkou paměti a tím tabulku přesměrovat bez nutnosti přepisovat flash paměť. 
...
Jedním z nejjednodušších možností, jak z programu uvnitř mikrokontroléru TMS570 pozdravit okolní svět, je řešení pomocí periferie SCI (sériového komunikačního rozhraní).
...
Dále je pak před každým dalším zápisem nutno kontrolovat příznak, jestli je v periferii místo na zapsání dalšího znaku.
...
Po restartu mikrokontroléru jsou parametry sériové komunikace nastaveny na použití jednoho stopbitu a bez použití parity a s délkou přenášených dat 1 bit.
Toto nastavení nemusí odpovídat v aplikací požadovanému nastavení a proto je potřeba přidat podporu mechanizmu, kterým si aplikace může zažádat o nastavení požadovaných parametrů.
...
V této funkci je potřeba inicializovat termios a zaregistrovat sériové porty, které zařízení podporuje.
...
Podpora funkce printk je přidána do druhého souboru a oddělena od ovladače sériových portů.
...
Pro účely zvýšení efektivity a vytíženosti pinů byla vytvořena transformační síť, která dovoluje výhradní kontrolu nad pinem a jeho přiřazení několika spjatým periferíjím.
...
Vektorový modul přerušení (VIM) je zásadní pro asynchronní obsluhu vnějších událostí.
Modul VIM rozhoduje o prioritě jednotlivých přerušení, informuje o posledním přerušení a řídí možnosti probuzení po události.

Z teorie je zřejmé, že jádro mikrokontroléru CORTEX-R4 podporuje práci s přerušeními.
CORTEX-4R rozděluje přerušení na rychlé (FIQ) a nornální (IRQ).
...
Přerušení je automaticky vyhodnoceno a vykonávání programu se přesune na příslušnou oblužnou rutinu vyplněnou v tabulce přerušení bez skoku na stadrarní obsluhy přerušení IRQ a FIQ v tabulce výjimek jádra CORTEX-R4.
...
Připojení se provede zapsáním instrukce skoku na funkci \_ARMV4\_Exception\_interrupt do tabulky výjimek jádra CORTEX-R4 (adresa 24 - IRQ a 28 - FIQ).
...
Toto řešení je vhodno pro co nejrychlejší obsluhu přerušení bez zbytečné zátěže, ale má nevýhodu, že nebudou provedené případné požadavky na přeplánování vzniklé během obsluhy přerušení.
Návrat z obslužné rutiny nebo driveru je skokem přímo do kódu přerušené úlohy. Systém nemá nad obsluhou přerušení dohled.
...
Funkce bsp\_interrupt\_dispatch musí rozhodnout o původu přerušení a předat řízení funkci bsp\_interrupt\_handler\_dispatch.
...
Přizpůsobení operačnímu systému je nejjednodušší část.
...
Výjimku tvoří funkce čekání na semafor a frontu.
...
Pro účely této práce byly standartní chybové kódy použity z operačního systému, tak aby nedocházelo ke kolizím ve jménech.
...
Veškerá komunikace tedy prochází touto dvojicí kanálů, je zpracovávána periferií EMAC a předána do knihovny LwIP.
...
Současná verze již licenčně umožňuje sloučení s projektem RTEMS.
...
Cenou za rozšíření této struktury o odkaz na víc je snížení maximálního množství dostupných struktur o čtvrtinu a zhoršení přenositelnosti kódu na jiný mikrokontroler s jiným typem ETHERNET periferie.

Pro účely přenášení dat je v systému při inicializaci ovladače zaregistrováno další vlákno a semafor.
Při přerušení od periferie EMAC je nastaven semafor, na který čeká zmíněné vlákno.
Tímto způsobem je dosaženo minimální doby strávené v přerušení a zvýšení suverenity operačního systému nad řízením běhu aplikace.
...
Poněkud odlišný způsob vnitřní reprezentace zmiňovaných zřetězených struktur tkví v udržování zřetězeného zacykleného seznamu všech struktur.
V něm si udržujeme odkaz na první připravenou a první volnou strukturu.
Protože je potřeba udržovat o několik odkazů méně, je tento způsob méně paměťově náročný.
Avšak klade mnohonásobně větší nároky na strojový čas, protože je neustále potřeba udržovat konzistenci a smysluplnost zmíněného cyklického seznamu.
...
Tato struktura obsahuje záhlaví již zmíněných zřetězených seznamů struktur emac\_tx\_bd a další interní informace jako jsou odkazy na periferie EMAC a MDIO.
...
Stejně tak kritické je správně uvolňovat paměť po špatné alokaci struktury pbuf.
